...
const CategorysOut = () => {
  ...
  const APIPost = "https://back-my-wallet-render.onrender.com/chart-out";
  const body = {
  ...
  const ApiGet = "https://back-my-wallet-render.onrender.com/chart-out";
  const axiosFunction = () => {
    const config = { headers: { Authorization: `Bearer ${data.token}` } };
    ...
    <Page>
      <Content>
        ...
        <TransitionArea>
          <Description>
            {categorys.length === 0 ? (
              <Message>Não há registros categorias ainda!</Message>
            ) : (
              categorys.map((e, index) => {
                return (
                  <CategoryItem
                    key={index}
                    description={e.descriptionCategory}
                  />
                );
              })
            )}
